...
- An assayData Table: The assayData table is generated after the
  identification and quantification of the MS spectra by a
  pre-processing software such as MaxQuant, ProteomeDiscoverer or
  MSFragger .
...
`D`). Check with this preview if the conversion process went fine. By
clicking on a row (corresponding to an assay), the selected assay will
appear below, once again check if everything looks as expected.

Once everything looks fine, you can download the QFeatures to use it
with R or continue the analysis using
...
destination folder, the name of the folder and click on save. Your
